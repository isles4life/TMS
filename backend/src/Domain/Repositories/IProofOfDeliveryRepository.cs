...
{
    Task<PODPhoto?> GetByIdAsync(Guid id);
    Task<List<PODPhoto>> GetByProofOfDeliveryIdAsync(Guid podId);
    Task<List<PODPhoto>> GetByTypeAsync(Guid podId, int photoType);
    
    Task<PODPhoto> CreateAsync(PODPhoto photo);
    Task<bool> DeleteAsync(Guid id);
    Task<bool> DeleteAllByProofOfDeliveryIdAsync(Guid podId);

    /// <summary>
    /// Get total file size of all photos in a POD (for quota management)
    /// </summary>
    Task<long> GetTotalFileSizeAsync(Guid podId);
}
